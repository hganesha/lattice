...
Run `pnpm generate:ontologies` after adding or changing industry forms under `../Schemas` or the repository-owned `schemas/` overrides. The generator currently derives eight provenance-backed industry ontologies from 63 implemented forms and publishes a field-coverage report in `docs/generated-ontology-report.json`. See [form-schema ontology generation](docs/ontology-generation.md).
