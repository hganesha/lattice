...
Use **Appearance** in the header to select the operating-system theme, light mode, or dark mode and to enable the larger text scale. Both preferences are stored locally in the browser. The contrast-bearing foreground/background theme tokens meet WCAG AA contrast for normal text; the default interface type floor is 12px, with a 13.5px large-text option.

The Studio ships with an English (`en-US`) source catalog, a Spanish (`es-ES`) translation, and a runtime-generated expanded pseudo-locale (`en-XA`) for finding untranslated text and layouts that cannot accommodate longer translations. Choose a language under **Appearance**; the preference is stored in the browser, and dates, times, numbers, and plurals use the active locale. Run `pnpm --filter @lattice/studio i18n:check` to extract and compile the source catalog. User-authored ontology content and server-provided evidence remain source data rather than being silently translated. RTL layout is not yet certified; see [docs/localization.md](docs/localization.md).

The Studio opens on the active industry workspace's **Shared ontology**. Fundamental entities and relationships are authored once at workspace level. **Contracts** then shows the decision contracts layered on that foundation; the active contract selector controls compiler, bindings, policy, assurance, evidence, and release views. Creating a contract inside a workspace now includes an explicit concept-scope step so a contract inherits only the Core and industry concepts relevant to its decisions.

Open **Runtime map** to inspect governed objects and relationships in a domain-neutral graph or table, trace their evidence, and compile the contract's competency question. The Grid example resolves its outage, traverses the governed `AFFECTED_ASSET` relationship, and returns a short-lived Ed25519-signed execution plan pinned to contract `0.1.1`.

Within a contract, drag types to arrange the canvas, draw between node handles to create relationships, edit properties in the inspector, then save the draft to the local registry or publish an immutable version. **Import schema** accepts OpenAPI or JSON Schema as YAML/JSON, previews types and references, and lets an author merge, create, or skip every collision before staging an unpublished draft. Switch to **Runtime map** to compile questions against that contract's latest published release.
