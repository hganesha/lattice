...
export function ReleaseManagementStudio({ contract, onRegistryChange, onManageDraft }: ReleaseManagementStudioProps) {
  ...
  return <section className="release-management-page">
    ...
    <div className="release-layout">
      ...
      <aside className="release-detail panel">{selectedRelease ? <>
        <header><div><span className="panel-kicker">{t('releaseInspector').toLocaleUpperCase()}</span><h2>v{selectedRelease.version}</h2></div><span className={isSelectedActive ? 'active-release' : ''}>{isSelectedActive ? t('releaseLive').toLocaleUpperCase() : t('releaseHistorical').toLocaleUpperCase()}</span></header>
