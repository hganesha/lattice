...
export function ReleaseManagementStudio({ contract, onRegistryChange, onManageDraft }: ReleaseManagementStudioProps) {
  ...
  return <section className="release-management-page">
    <div className="surface-hero tint-warning"><div><span className="panel-kicker">{t('releaseManagementKicker').toLocaleUpperCase()}</span><h2>{t('releaseManagementTitle')}</h2><p>{t('releaseManagementDescription')}</p></div><div className="surface-hero-actions"><span className={`runtime-state ${(entry?.runtimeStatus ?? 'NO_RELEASE').toLocaleLowerCase()}`}><i />{entry?.runtimeStatus ? t('releaseRuntimeStatus', { status: entry.runtimeStatus.replaceAll('_', ' ') }) : t('commonLoading')}</span>{entry?.runtimeStatus === 'ACTIVE' ? <button className="danger-ghost" onClick={() => void changeRuntimeStatus('SUSPENDED')} disabled={working}>{t('releaseSuspendRuntime')}</button> : entry?.runtimeStatus === 'SUSPENDED' ? <button className="release" onClick={() => void changeRuntimeStatus('ACTIVE')} disabled={working}>{t('releaseResumeRuntime')}</button> : null}</div></div>
