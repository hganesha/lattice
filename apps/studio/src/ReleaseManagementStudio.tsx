...
export function ReleaseManagementStudio({ contract, onRegistryChange, onManageDraft }: ReleaseManagementStudioProps) {
  ...
  return <section className="release-management-page">
    <div className="surface-hero tint-warning"><div><span className="panel-kicker">{t('releaseManagementKicker')}</span><h2>{t('releaseManagementTitle')}</h2><p>{t('releaseManagementDescription')}</p></div><div className="surface-hero-actions"><span className={`runtime-state ${(entry?.runtimeStatus ?? 'NO_RELEASE').toLocaleLowerCase()}`}><i />{entry?.runtimeStatus ? t('releaseRuntimeStatus', { status: entry.runtimeStatus.replaceAll('_', ' ') }) : t('commonLoading')}</span>{entry?.runtimeStatus === 'ACTIVE' ? <button className="danger-ghost" onClick={() => void changeRuntimeStatus('SUSPENDED')} disabled={working}>{t('releaseSuspendRuntime')}</button> : entry?.runtimeStatus === 'SUSPENDED' ? <button className="release" onClick={() => void changeRuntimeStatus('ACTIVE')} disabled={working}>{t('releaseResumeRuntime')}</button> : null}</div></div>
    {notice && <Toast message={notice} closeLabel={t('commonClose')} onDismiss={() => setNotice('')} />}
    <div className="release-stats"><article><span>{t('releaseImmutableReleases')}</span><b>{entry?.releases.length ?? 0}</b><small>{t('releaseAppendOnly')}</small></article><article><span>{t('releaseActiveVersion')}</span><b>{activeRelease ? `v${activeRelease.version}` : '—'}</b><small>{entry?.runtimeStatus === 'SUSPENDED' ? t('releaseRuntimeSuspended') : t('releaseServingCompilation')}</small></article><article><span>{t('releaseDraftDelta')}</span><b>{changes.length}</b><small>{t('releaseAgainstSelected')}</small></article><article><span>{t('releaseSuggestedBump')}</span><b>{suggestedBump}</b><small>{t('releaseBasedOnImpact')}</small></article></div>
    <div className={`release-layout ${inspectorCollapsed ? 'inspector-collapsed' : ''}`}>
      <main className="release-timeline panel"><header><div><span className="panel-kicker">{t('releaseTimeline')}</span><h2>{contract.name}</h2></div><button className="release" onClick={onManageDraft}>{t('releaseManageDraft')} →</button></header><div className="release-list-full">{entry?.releases.length === 0 && <EmptyState icon="◇" title={t('releaseEmptyTitle')} description={t('releaseEmptyDescription')} />}{entry?.releases.slice().reverse().map((release) => <button className={`surface-row lead flush interactive block release-card ${selectedDigest === release.digest ? 'selected' : ''}`} onClick={() => { setSelectedDigest(release.digest); setRollbackOpen(false); setRollbackRationale('') }} key={release.digest}><span className="release-node"><i /></span><div><span><b>v{release.version}</b>{release.digest === entry.activeReleaseDigest && <em>{t('releaseActive')}</em>}</span><h3>{release.notes}</h3><footer><time>{formatDate(release.publishedAt, { dateStyle: 'medium', timeStyle: 'short' })}</time><code>{release.digest.slice(0, 28)}…</code></footer></div><dl><div><dt>{t('contractsTypes')}</dt><dd>{release.contract.entityTypes.length}</dd></div><div><dt>{t('releaseBindings')}</dt><dd>{release.contract.bindings.length}</dd></div><div><dt>{t('releasePolicies')}</dt><dd>{release.contract.policies.length}</dd></div></dl></button>)}</div></main>
      <aside className={`release-detail collapsible-inspector panel ${inspectorCollapsed ? 'collapsed' : ''}`} id="release-inspector">
        <div className="collapsible-inspector-header">
          {!inspectorCollapsed && selectedRelease && <header><div><span className="panel-kicker">{t('releaseInspector')}</span><h2>v{selectedRelease.version}</h2></div><span className={isSelectedActive ? 'active-release' : ''}>{isSelectedActive ? t('releaseLive') : t('releaseHistorical')}</span></header>}
        ...
        <div className="release-digest"><span>{t('releaseImmutableDigest')}</span><code>{selectedRelease.digest}</code></div>
        <div className="release-notes"><span>{t('releaseNotes')}</span><p>{selectedRelease.notes}</p></div>
        <dl className="release-versions"><div><dt>{t('releaseVersionSemantic')}</dt><dd>{selectedRelease.contract.versions.semantic}</dd></div><div><dt>{t('releaseVersionPolicy')}</dt><dd>{selectedRelease.contract.versions.policy}</dd></div><div><dt>{t('releaseVersionBindings')}</dt><dd>{selectedRelease.contract.versions.bindings}</dd></div><div><dt>{t('releaseVersionApi')}</dt><dd>{selectedRelease.contract.versions.api}</dd></div></dl>
    ...
    <section className="release-impact panel"><header><div><span className="panel-kicker">{t('releaseDiffImpact')}</span><h2>{selectedRelease ? comparisonDigest === 'WORKING_DRAFT' ? t('releaseWorkingComparison', { version: selectedRelease.version }) : t('releaseReleaseComparison', { from: selectedRelease.version, to: entry?.releases.find((release) => release.digest === comparisonDigest)?.version ?? '—' }) : t('releaseSelectBaseline')}</h2></div>{selectedRelease && <div className="release-diff-controls"><label>{t('releaseCompareTo')}<select value={comparisonDigest} onChange={(event) => setComparisonDigest(event.target.value)}><option value="WORKING_DRAFT">{t('releaseWorkingDraft')}</option>{entry?.releases.map((release) => <option key={release.digest} value={release.digest}>v{release.version}</option>)}</select></label>{releaseDiff && <button className="ghost" onClick={downloadDiff}>{t('releaseDownloadDiff')}</button>}<span className={`bump-pill ${suggestedBump.toLocaleLowerCase()}`}>{t('releaseVersionBump', { bump: suggestedBump })}</span></div>}</header>{diffLoading ? <div className="diff-empty"><div><b>{t('commonLoading')}</b></div></div> : changes.length === 0 ? <div className="diff-empty"><span>✓</span><div><b>{t('releaseNoDifferences')}</b><p>{t('releaseEquivalent')}</p></div></div> : <div className="change-grid">{changes.map((change) => <article key={`${change.kind}:${change.id}`}><span className={change.change.toLocaleLowerCase()}>{change.change[0]}</span><div><small>{changeKindLabel(change.kind, t)}</small><b>{change.label}</b><code>{change.id}</code></div><em className={change.impact.toLocaleLowerCase()}>{change.impact}</em></article>)}</div>}</section>
