...
export function BindingEditor({ contract, workspaceId, onCancel, onApply }: BindingEditorProps) {
  ...
  const supportsLiveDiscovery = connector?.id === 'DATABRICKS' || connector?.id === 'POSTGRESQL'
  ...
  function selectConnector(provider: ConnectorProvider) {
    ...
    setSourceName('')
    setSourceText('')
    setLiveDiscovery(provider === 'DATABRICKS' || provider === 'POSTGRESQL')
  ...
  function loadTabularExample() {
    ...
    setQueryTemplate(connector?.id === 'POSTGRESQL' ? 'SELECT event_id, severity, started_at, asset_id, customers_affected FROM grid.current_outages WHERE event_id = $1' : 'SELECT event_id, severity, started_at, asset_id, customers_affected FROM operations.current_outages WHERE event_id = :event_id')
