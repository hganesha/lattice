...
export function BindingEditor({ contract, workspaceId, onCancel, onApply }: BindingEditorProps) {
  ...
  const supportsLiveDiscovery = connector?.id === 'DATABRICKS' || connector?.id === 'POSTGRESQL' || connector?.id === 'MICROSOFT_FABRIC'
  ...
  function selectConnector(provider: ConnectorProvider) {
    ...
    setSourceName('')
    setSourceText('')
    setLiveDiscovery(provider === 'DATABRICKS' || provider === 'POSTGRESQL' || provider === 'MICROSOFT_FABRIC')
  ...
  function loadTabularExample() {
    ...
    setBaseUrl(connector?.id === 'DATABRICKS' ? 'https://dbc-example.cloud.databricks.com' : connector?.id === 'POSTGRESQL' ? 'postgresql://db.example.internal:5432/operations' : connector?.id === 'SNOWFLAKE' ? 'https://example.snowflakecomputing.com' : connector?.id === 'MICROSOFT_FABRIC' ? 'workspace.datawarehouse.fabric.microsoft.com' : 'data.example.internal')
    ...
    setQueryTemplate(connector?.id === 'POSTGRESQL' ? 'SELECT event_id, severity, started_at, asset_id, customers_affected FROM grid.current_outages WHERE event_id = $1' : connector?.id === 'MICROSOFT_FABRIC' ? 'SELECT event_id, severity, started_at, asset_id, customers_affected FROM grid.current_outages WHERE event_id = @event_id' : 'SELECT event_id, severity, started_at, asset_id, customers_affected FROM operations.current_outages WHERE event_id = :event_id')
  ...
  async function analyze() {
    ...
    setError('')
    try {
      const useLiveDiscovery = supportsLiveDiscovery && liveDiscovery
      const draftOperationId = `${slugify(sourceSystem)}.${connector?.operationVerb.toLocaleLowerCase()}_${slugify(sourceName)}`
      const discoveryBinding: SourceBinding | undefined = connector ? {
        id: 'binding_discovery_preview',
        sourceSystem: sourceSystem.trim(),
        operationId: draftOperationId,
        environment,
        freshnessMinutes,
        requiredPermissions: permission.split(',').map((item) => item.trim()).filter(Boolean),
        expectedResultSchema: `${slugify(sourceName)}_row`,
        version: '0.1.0',
      ...
      const response = await fetch(`${API_URL}${useLiveDiscovery ? '/v1/connectors/discover' : '/v1/bindings/preview'}`, {
        method: 'POST',
        headers: { ...apiAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify(useLiveDiscovery ? {
          ...(workspaceId ? { workspaceId } : { contractId: contract.id }), sourceName, binding: discoveryBinding,
        } : {
          ...(workspaceId ? { workspaceId } : { contractId: contract.id }), sourceName, sourceText,
          format: connector?.id === 'OPENAPI' ? 'OPENAPI' : 'TABULAR_SCHEMA',
          ...(connector?.id === 'OPENAPI' ? {} : { operationId: draftOperationId, operationLabel: `${connector?.operationVerb === 'QUERY' ? 'Query' : connector?.operationVerb === 'SUBSCRIBE' ? 'Subscribe to' : 'Read'} ${sourceName}` }),
        }),
      })
