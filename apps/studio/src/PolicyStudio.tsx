...
import { EmptyState, SurfaceHero } from './SurfaceState'
import { policyPinFact, releaseFact } from './surfaceContext'
...
export function PolicyStudio({ contract, onChange, onDirtyChange }: PolicyStudioProps) {
  ...
  return <section className="policy-studio-page">
    <SurfaceHero
      kicker={t('policyStudioKicker')}
      title={contract.name}
      description={t('policyStudioDescription')}
      facts={[releaseFact(t, contract), policyPinFact(t, contract)]}
      tint="governance"
    >
      <button className="ghost" onClick={addRecommendedBaseline}>＋ {t('policyRecommendedBaseline')}</button>
      <button className="release" onClick={() => setEditingPolicy('NEW')}>{t('policyCreate')} →</button>
    </SurfaceHero>
    {notice && <Toast message={notice} closeLabel={t('commonClose')} onDismiss={() => setNotice('')} />}
    <div className="policy-stats"><article><span>{t('policyProfiles')}</span><b>{contract.policies.length}</b><small>{t('policyVersionPinned')}</small></article><article><span>{t('policyRiskCoverage')}</span><b>{coveredTiers.length} / {requiredTiers.length}</b><small>{t('policyOperationTiers')}</small></article><article><span>{t('policyApproved')}</span><b>{approvedCount} / {contract.policies.length}</b><small>{t('policyReleaseEligible')}</small></article><article><span>{t('policyHumanEscalation')}</span><b>{escalationCount}</b><small>{t('policyApprovalGated')}</small></article></div>
    <div className="policy-layout">
      <main className="policy-profiles panel"><header><div><span className="panel-kicker">{t('policyGuardrailProfiles')}</span><h2>{t('policyProfilesTitle')}</h2></div><span>{t('policyProfileCount', { count: contract.policies.length })}</span></header><div className="policy-profile-list">{contract.policies.length === 0 && <EmptyState icon="◈" title={t('policyEmptyTitle')} description={t('policyEmptyDescription')} actionLabel={`${t('policyAddBaseline')} →`} onAction={addRecommendedBaseline} />}{contract.policies.map((policy) => <article className="surface-row policy-card" key={policy.id}><div className="policy-tier-icon">{policy.riskTier.split('_').map((word) => word[0]).join('')}</div><div className="policy-card-main"><div><h3>{policy.label}</h3><span className={`claim-status ${policy.approvalStatus.toLocaleLowerCase()}`}>{policy.approvalStatus.replaceAll('_', ' ')}</span></div><p>{policy.description}</p><footer><code>{policy.id}</code><span>{t('policyOwner', { owner: policy.owner })}</span><span>v{policy.version}</span></footer></div><dl><div><dt>{t('policyEvidence')}</dt><dd>{policy.minimumEvidenceStrength}</dd></div><div><dt>{t('policyFreshness')}</dt><dd>≤ {formatAge(policy.maximumEvidenceAgeMinutes)}</dd></div><div><dt>{t('policyEscalation')}</dt><dd className={policy.approvalRequired ? 'required' : ''}>{policy.approvalRequired ? t('policyRequired') : t('policyAutonomous')}</dd></div></dl><button className="ghost" onClick={() => setEditingPolicy(policy)}>{t('commonEdit')}</button></article>)}</div></main>
      <aside className="policy-coverage panel"><div className="panel-header"><div><span className="panel-kicker">{t('policyRiskTierCoverage')}</span><h2>{t('policyOperationControls')}</h2></div></div><div className="policy-tier-list">{riskTiers.map((tier) => { const policy = contract.policies.find((item) => item.riskTier === tier); const required = operationCounts[tier] > 0; return <div key={tier} className={required && !policy ? 'gap' : ''}><span className={policy ? 'covered' : required ? 'missing' : 'unused'}>{policy ? '✓' : required ? '!' : '–'}</span><div><b>{tier.replaceAll('_', ' ')}</b><small>{t('policyOperationCount', { count: operationCounts[tier] })}</small></div><em>{policy ? policy.minimumEvidenceStrength : required ? t('policyNeeded') : t('policyNotUsed')}</em></div>})}</div><div className="policy-principle"><span>⌁</span><div><b>{t('policyExecutableContext')}</b><p>{t('policyExecutableDescription')}</p></div></div></aside>
    </div>
