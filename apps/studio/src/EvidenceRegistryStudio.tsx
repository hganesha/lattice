...
export function EvidenceRegistryStudio({ contract }: EvidenceRegistryStudioProps) {
  ...
  const dependencies = selected ? evidenceDependencies(contract, selected, reviews) : []
...
function evidenceDependencies(contract: ContextContract, evidence: EvidenceRecord, reviews: ReviewRequestArtifact[]): EvidenceDependency[] {
  ...
  for (const review of reviews.filter((item) => item.decision?.artifactDigest === evidence.checksum)) dependencies.push({ id: review.id, kind: 'DECISION', label: review.targetLabel, detail: `${review.decision?.decision.replaceAll('_', ' ')} by ${review.decision?.decidedBy}` })
  if (evidence.locator.startsWith('/v1/assurance/runs/')) dependencies.push({ id: evidence.locator, kind: 'ASSURANCE', label: 'Assurance run', detail: `${contract.tests.filter((test) => test.lastRun === evidence.observedAt).length} synchronized checks` })
  return dependencies
}
