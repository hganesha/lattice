...
export function EvidenceRegistryStudio({ contract }: EvidenceRegistryStudioProps) {
  ...
  return <section className="evidence-registry-page">
    <div className="surface-hero tint-info"><div><span className="panel-kicker">{t('evidenceRegistryKicker')}</span><h2>{t('evidenceRegistryTitle')}</h2><p>{t('evidenceRegistryDescription')}</p></div><div className="evidence-integrity"><span>⌁</span><div><b>{t('evidenceContentAddressed')}</b><small>{t('evidenceImmutableReferences', { count: contract.evidence.length })}</small></div></div></div>
    <div className="evidence-stats"><article><span>{t('evidenceTotalArtifacts')}</span><b>{contract.evidence.length}</b><small>{t('evidenceClasses', { count: types.length })}</small></article><article><span>{t('evidenceCurrent')}</span><b>{freshnessCounts.CURRENT}</b><small>{t('evidenceWithinDay')}</small></article><article><span>{t('evidenceAging')}</span><b>{freshnessCounts.AGING}</b><small>{t('evidenceWithinWeek')}</small></article><article><span>{t('evidenceDependencyLinks')}</span><b>{contract.evidence.reduce((total, evidence) => total + evidenceDependencies(contract, evidence, reviews, t).length, 0)}</b><small>{t('evidenceClaimsEvents')}</small></article></div>
    <div className={`evidence-layout ${inspectorCollapsed ? 'inspector-collapsed' : ''}`}>
      <main className="evidence-list-panel panel"><header><div><span className="panel-kicker">{t('evidenceProvenanceArtifacts')}</span><h2>{contract.name}</h2></div><nav aria-label={t('evidenceFilters')}><button className={filter === 'ALL' ? 'active' : ''} onClick={() => setFilter('ALL')}>{t('reviewFilterAll')}</button>{types.map((type) => <button className={filter === type ? 'active' : ''} onClick={() => setFilter(type)} key={type}>{shortType(type)}</button>)}</nav></header><div className="evidence-list">{visibleEvidence.map((evidence) => { const state = freshness(evidence, now); const linked = evidenceDependencies(contract, evidence, reviews, t).length; return <button className={`surface-row lead interactive block evidence-row ${selected?.id === evidence.id ? 'selected' : ''}`} onClick={() => setSelectedId(evidence.id)} key={evidence.id}><span className={`evidence-type-icon ${evidence.type.toLocaleLowerCase()}`}>{typeIcon(evidence.type)}</span><div><span><b>{evidence.title}</b><em className={state.toLocaleLowerCase()}>{freshnessLabel(state, t)}</em></span><p>{evidence.source} · {evidence.locator}</p><footer><code>{evidence.id}</code><span>{evidence.status.replaceAll('_', ' ')}</span><span>{t('evidenceDependencies', { count: linked })}</span></footer></div><time>{formatDate(evidence.observedAt, { dateStyle: 'medium', timeStyle: 'short' })}</time></button>})}</div></main>
      <aside className={`evidence-inspector collapsible-inspector panel ${inspectorCollapsed ? 'collapsed' : ''}`} id="evidence-inspector">
        <div className="collapsible-inspector-header">
          {!inspectorCollapsed && selected && <header><div><span className="panel-kicker">{t('evidenceDetail')}</span><h2>{selected.title}</h2></div><span className={`evidence-freshness ${freshness(selected, now).toLocaleLowerCase()}`}>{freshnessLabel(freshness(selected, now), t)}</span></header>}
        ...
        {!inspectorCollapsed && (selected ? <><div className="surface-row evidence-source-card"><span>{typeIcon(selected.type)}</span><div><b>{selected.source}</b><small>{selected.type.replaceAll('_', ' ')}</small></div></div><dl><div><dt>{t('evidenceObserved')}</dt><dd>{formatDate(selected.observedAt, { dateStyle: 'medium', timeStyle: 'short' })}</dd></div><div><dt>{t('evidenceValidFrom')}</dt><dd>{formatDate(selected.validFrom, { dateStyle: 'medium', timeStyle: 'short' })}</dd></div><div><dt>{t('evidenceValidUntil')}</dt><dd>{selected.validUntil ? formatDate(selected.validUntil, { dateStyle: 'medium', timeStyle: 'short' }) : t('evidenceOpenEnded')}</dd></div><div><dt>{t('evidenceClaimStatus')}</dt><dd>{selected.status.replaceAll('_', ' ')}</dd></div><div><dt>{t('evidenceLocator')}</dt><dd>{selected.locator}</dd></div></dl><div className="evidence-checksum"><span>{t('evidenceContentDigest')}</span><code>{selected.checksum}</code></div><section className="dependency-section"><div><span>{t('evidenceDependencyTrace')}</span><b>{dependencies.length}</b></div>{dependencies.length === 0 && <p>{t('evidenceNoDirectClaim')}</p>}{dependencies.map((dependency) => <article key={`${dependency.kind}:${dependency.id}`}><span>{dependency.kind.slice(0, 3)}</span><div><b>{dependency.label}</b><small>{dependency.detail}</small></div></article>)}</section></> : <div className="evidence-empty">{t('evidenceNoArtifacts')}</div>)}
      </aside>
    </div>
  </section>
}
