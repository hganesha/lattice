  ...
  return <div className="attestation-panel">
    ...
    {records.map((attestation) => {
      ...
      return <article className="attestation-card" key={attestation.id}>
        <header><div><span className="attestation-predicate">{attestation.predicateType}</span><code>{attestation.id}</code></div><div className="attestation-flags">{attestation.revocation ? <span className="disposition-chip danger"><IconBan /> {t('attestationRevoked')}</span> : <span className="disposition-chip neutral">{t('attestationNotRevoked')}</span>}{expired ? <span className="disposition-chip warning"><IconAlertTriangle /> {t('attestationExpired')}</span> : <span className="disposition-chip neutral">{t('attestationCurrent')}</span>}</div></header>
        ...
        <div className="attestation-result" role="status" aria-live="polite">
          ...
          {verification && <>
            ...
            <ul className="attestation-checks" aria-label={t('attestationChecksLabel')}>{verification.checks.map((check) => <li className={check.status.toLocaleLowerCase()} key={check.id}><span className="attestation-check-name">{t(checkKeys[check.id])}</span><span className={`disposition-chip ${check.status === 'PASS' ? 'success' : check.status === 'FAIL' ? 'danger' : 'warning'}`}>{check.status === 'PASS' ? t('attestationStatusPass') : check.status === 'FAIL' ? t('attestationStatusFail') : t('attestationStatusUnavailable')}</span><span className="attestation-check-message">{check.message}</span></li>)}</ul>
          </>}
        </div>
      </article>
    })}
