...
export function ReviewDecisionPanel({ review, onClose, onDecided }: ReviewDecisionPanelProps) {
  ...
  return <Overlay variant="drawer" eyebrow={t('reviewDecisionKicker')} title={t('reviewDecisionTitle', { label: review.targetLabel })} closeLabel={t('reviewClosePanel')} onClose={onClose} dismissOnBackdrop={false}>
      <p className="review-impact">{t('reviewImpact', { kind: review.targetKind.replaceAll('_', ' ').toLocaleLowerCase(), impact: review.impact })}</p>
      ...
      <form className="review-decision-form" onSubmit={(event) => void submit(event)}>
        ...
        <div className="overlay-actions"><button type="button" className="ghost" onClick={onClose}>{t('commonCancel')}</button><button className="release" type="submit" disabled={submitting || rationale.trim().length < 12}>{submitting ? t('reviewRecording') : t('reviewRecordDecision')}</button></div>
      </form>
  </Overlay>
}
