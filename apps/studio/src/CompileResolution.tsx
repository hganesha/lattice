...
export function CompileResolution({ result, onChoose }: CompileResolutionProps) {
  ...
  return <section className={`resolution ${resolved ? 'resolved' : 'attention'}`}>
    ...
    {result.grounding === 'SIMULATED' && <div className="signed-pill simulated"><span>{t('runtimeSimulatedGrounding').toLocaleUpperCase()}</span><small>{t('runtimeSimulatedGroundingDetail')}</small></div>}
