...
export function ReviewQueueStudio({ contract, onChange, onDirtyChange }: ReviewQueueStudioProps) {
  ...
  const claims = useMemo<ReviewClaim[]>(() => [
    ...contract.entityTypes.map((type) => ({ kind: 'ENTITY_TYPE' as const, id: type.id, label: type.label, description: type.description, status: reviews.some((review) => review.targetKind === 'ENTITY_TYPE' && review.targetId === type.id && review.status === 'OPEN') ? 'IN_REVIEW' as const : type.approvalStatus, impact: type.impact, detail: `${type.properties.length} governed properties`, evidenceRefs: contract.evidence.filter((evidence) => evidence.status === type.evidenceStatus || evidence.status === 'TEMPLATE_DERIVED').slice(-3).map((evidence) => evidence.id) })),
    ...contract.bindings.map((binding) => ({ kind: 'SOURCE_BINDING' as const, id: binding.id, label: binding.sourceSystem, description: `${binding.method ?? 'OP'} ${binding.endpoint ?? binding.operationId}`, status: reviews.some((review) => review.targetKind === 'SOURCE_BINDING' && review.targetId === binding.id && review.status === 'OPEN') ? 'IN_REVIEW' as const : binding.approvalStatus, impact: 'HIGH' as const, detail: `${binding.mappings?.length ?? 0} mappings · freshness ≤ ${binding.freshnessMinutes} min`, evidenceRefs: contract.evidence.filter((evidence) => evidence.checksum === binding.sourceChecksum || evidence.locator.includes(binding.sourceSystem)).map((evidence) => evidence.id) })),
    ...contract.policies.map((policy) => ({ kind: 'POLICY' as const, id: policy.id, label: policy.label, description: policy.description, status: reviews.some((review) => review.targetKind === 'POLICY' && review.targetId === policy.id && review.status === 'OPEN') ? 'IN_REVIEW' as const : policy.approvalStatus, impact: policy.riskTier === 'OPERATIONAL_ACTION' ? 'CRITICAL' as const : policy.riskTier === 'PLANNING_DECISION' ? 'HIGH' as const : 'MEDIUM' as const, detail: `${policy.minimumEvidenceStrength} evidence · freshness ≤ ${policy.maximumEvidenceAgeMinutes} min`, evidenceRefs: contract.evidence.filter((evidence) => evidence.locator.includes(policy.id)).map((evidence) => evidence.id) })),
  ], [contract.bindings, contract.entityTypes, contract.evidence, contract.policies, reviews])
...
function DecisionDetail({ review, onClose }: { review: ReviewRequestArtifact; onClose: () => void }) {
  ...
  return <div className="modal-backdrop review-decision-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="decision-detail" role="dialog" aria-modal="true" aria-labelledby="decision-detail-title"><header><div><span className="panel-kicker">IMMUTABLE DECISION</span><h2 id="decision-detail-title">{review.targetLabel}</h2></div><button aria-label="Close decision detail" onClick={onClose}>×</button></header><div className={`decision-seal ${decision.decision.toLocaleLowerCase()}`}><span>{decision.decision === 'REJECTED' ? '×' : '✓'}</span><b>{decision.decision.replaceAll('_', ' ')}</b></div><blockquote>{decision.rationale}</blockquote><dl><div><dt>Reviewer</dt><dd>{decision.decidedBy}</dd></div><div><dt>Decided</dt><dd>{new Date(decision.decidedAt).toLocaleString()}</dd></div><div><dt>Target</dt><dd>{review.targetKind.replaceAll('_', ' ')}</dd></div></dl><div className="decision-digest"><span>DECISION DIGEST</span><code>{decision.artifactDigest}</code></div><footer><button className="release" onClick={onClose}>Done</button></footer></section></div>
}
