...
export function ReviewQueueStudio({ contract, onChange, onDirtyChange }: ReviewQueueStudioProps) {
  ...
  return <section className="review-queue-page">
    <div className="surface-hero tint-warning"><div><span className="panel-kicker">{t('reviewQueueKicker')}</span><h2>{t('reviewQueueTitle')}</h2><p>{t('reviewQueueDescription')}</p></div></div>
    {notice && <Toast message={notice} closeLabel={t('commonClose')} onDismiss={() => setNotice('')} />}
    <div className="review-stats"><ReviewMetric label={t('reviewActionRequired')} value={String(actionCount)} meta={t('reviewDraftRejected')} tone="warning" /><ReviewMetric label={t('reviewOpenReviews')} value={String(openCount)} meta={t('reviewAwaitingDecision')} tone="info" /><ReviewMetric label={t('reviewApproved')} value={`${approvedCount} / ${claims.length}`} meta={t('policyReleaseEligible')} tone="success" /><ReviewMetric label={t('reviewDecisionArtifacts')} value={String(reviews.filter((review) => review.status === 'DECIDED').length)} meta={t('reviewImmutableHistory')} tone="brand" /></div>
    <div className="review-layout">
      <main className="review-claims panel"><header><div><span className="panel-kicker">{t('reviewContractClaims')}</span><h2>{contract.name}</h2></div><nav aria-label={t('reviewFilters')}>{(['ACTION_REQUIRED', 'OPEN', 'DECIDED', 'ALL'] as const).map((item) => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{t(item === 'ACTION_REQUIRED' ? 'reviewActionRequired' : item === 'OPEN' ? 'reviewFilterOpen' : item === 'DECIDED' ? 'reviewFilterDecided' : 'reviewFilterAll')}</button>)}</nav></header><div className="review-claim-list">{visibleClaims.length === 0 && <EmptyState icon="✓" title={t('reviewEmptyTitle')} description={t('reviewEmptyDescription')} />}{visibleClaims.map((claim) => { const review = reviews.find((item) => item.targetKind === claim.kind && item.targetId === claim.id && (item.status === 'OPEN' || claim.status !== 'IN_REVIEW')); return <article className="surface-row lead review-claim" key={`${claim.kind}:${claim.id}`}><div className={`claim-kind ${claim.kind.toLocaleLowerCase()}`}>{claim.kind === 'ENTITY_TYPE' ? 'TYPE' : claim.kind === 'POLICY' ? 'POL' : 'API'}</div><div className="claim-main"><div><h3>{claim.label}</h3><span className={`claim-status ${claim.status.toLocaleLowerCase()}`}>{claim.status.replaceAll('_', ' ')}</span><em>{claim.impact}</em></div><p>{claim.description}</p><footer><code>{claim.id}</code><span>{claim.detail}</span><span>{t('reviewEvidenceRefs', { count: claim.evidenceRefs.length })}</span></footer></div><div className="claim-action">{['DRAFT', 'REJECTED'].includes(claim.status) && <button className="release" onClick={() => void submitForReview(claim)} disabled={submittingId === claim.id}>{submittingId === claim.id ? t('reviewSubmitting') : `${t('reviewSubmit')} →`}</button>}{claim.status === 'IN_REVIEW' && review && <button className="review-button" onClick={() => setSelectedReview(review)}>{t('reviewOpen')} →</button>}{['APPROVED', 'APPROVED_WITH_EXCEPTION'].includes(claim.status) && review?.decision && <button className="history-button" onClick={() => setSelectedReview(review)}>{t('reviewViewDecision')}</button>}</div></article> })}</div></main>
      <aside className="review-history panel"><div className="panel-header"><div><span className="panel-kicker">{t('reviewDecisionLedger')}</span><h2>{t('reviewRecentEvents')}</h2></div></div><div className="review-history-list">{reviews.length === 0 && <p>{t('reviewNoArtifacts')}</p>}{reviews.slice(0, 8).map((review) => <button onClick={() => setSelectedReview(review)} key={review.id}><i className={review.status.toLocaleLowerCase()} /><div><b>{review.targetLabel}</b><span>{review.status === 'OPEN' ? t('reviewSubmittedByName', { name: review.submittedBy }) : t('reviewDecidedByName', { decision: review.decision?.decision.replaceAll('_', ' ') ?? '', name: review.decision?.decidedBy ?? '' })}</span><time>{formatDate(review.decision?.decidedAt ?? review.submittedAt, { dateStyle: 'medium', timeStyle: 'short' })}</time></div></button>)}</div><div className="review-integrity"><span>⌁</span><div><b>{t('reviewAppendOnly')}</b><p>{t('reviewAppendOnlyDescription')}</p></div></div></aside>
    </div>
...
function ReviewMetric({ label, value, meta, tone }: { label: string; value: string; meta: string; tone: string }) {
  return <div className="surface-row review-metric"><div><span>{label}</span><i className={`mini-dot ${tone}`} /></div><b>{value}</b><small>{meta}</small></div>
}
