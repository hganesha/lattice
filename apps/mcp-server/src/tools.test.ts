  ...
  const { client, calls } = await connect({
    'POST /v1/plans/plan-1/execute': {
      ...
      body: {
        ...
        bindingResults: [{ bindingId: 'b1', sourceSystem: 'Risk warehouse', mode: 'CONNECTOR', status: 'SUCCESS', durationMs: 4, mappedValues: [{ sourcePath: '$.x', targetTypeId: 'counterparty', targetPropertyId: 'counterparty.exposure', value: 42, disclosure: 'VALUE', classification: 'INTERNAL' }] }],
