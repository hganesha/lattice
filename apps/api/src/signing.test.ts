...
test('a plan signed before a rotation still verifies afterwards', () => {
  ...
  const signature = beforeRotation.sign(payload)
...
test('a retired key cannot be passed off as the current one', () => {
  const previous = generateKeyPairSync('ed25519')
  const current = generateKeyPairSync('ed25519')
  const payload = Buffer.from('{"planId":"plan-1"}')
  const signer = new LocalPlanSigner(current.privateKey, [previous.publicKey])

  const signedByRetired = new LocalPlanSigner(previous.privateKey).sign(payload)
...
test('an unknown key identifier is rejected rather than tried against every key', () => {
  const { privateKey } = generateKeyPairSync('ed25519')
  const signer = new LocalPlanSigner(privateKey)
  const signature = signer.sign(Buffer.from('x'))
...
test('a configured key is used, and its absence is reported as ephemeral', () => {
  const { privateKey, publicKey } = generateKeyPairSync('ed25519')

  const configured = planSignerFromEnvironment({ LATTICE_SIGNING_KEY: pem(privateKey) })
  assert.equal(configured.ephemeral, false)
  assert.equal(configured.signer.activeKeyId, keyThumbprint(publicKey))

  const generated = planSignerFromEnvironment({})
  assert.equal(generated.ephemeral, true)
})

test('a base64-encoded key is accepted for environments that mangle newlines', () => {
  const { privateKey, publicKey } = generateKeyPairSync('ed25519')
  const encoded = Buffer.from(pem(privateKey)).toString('base64')

  const result = planSignerFromEnvironment({ LATTICE_SIGNING_KEY: encoded })
  assert.equal(result.signer.activeKeyId, keyThumbprint(publicKey))
})

test('retired keys are loaded so a rotation does not invalidate live plans', () => {
  const previous = generateKeyPairSync('ed25519')
  const current = generateKeyPairSync('ed25519')
  const retiredPem = previous.publicKey.export({ type: 'spki', format: 'pem' }).toString()

  const { signer } = planSignerFromEnvironment({
...
test('malformed key material fails loudly at startup', () => {
  assert.throws(() => planSignerFromEnvironment({ LATTICE_SIGNING_KEY: 'not-a-key' }), /PKCS#8 Ed25519 private key/)
  assert.throws(
