...
test('persists immutable assurance artifacts', async () => {
  ...
  assert.equal((await store.list(counterpartyRiskContract.id, 'tenant_a'))[0]?.artifactDigest, run.artifactDigest)
...
test('assurance artifacts are not readable from another tenant', async () => {
  ...
  assert.equal((await store.list(counterpartyRiskContract.id, 'tenant_b')).length, 0)
  assert.equal(await store.get(run.id, 'tenant_b'), undefined)
  assert.equal((await store.get(run.id, 'tenant_a'))?.id, run.id)
})
